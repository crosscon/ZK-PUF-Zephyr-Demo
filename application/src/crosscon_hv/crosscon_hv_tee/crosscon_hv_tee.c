...
{
    /* raw payload dump via ipc_shm.addr */
    volatile uint8_t *payload_ptr = (volatile uint8_t *)ipc_shm.addr;
    uint8_t payload_copy[VMS_IPC_PAYLOAD_SIZE];
    memcpy(payload_copy, payload_ptr, VMS_IPC_PAYLOAD_SIZE);
    LOG_HEXDUMP_DBG(payload_copy, VMS_IPC_PAYLOAD_SIZE, "Payload Copy");
