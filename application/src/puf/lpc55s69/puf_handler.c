 ...
 * the compiler won’t optimize it away.
 */
...
TEE_Result puf_get_key(const uint8_t *label, size_t label_len, uint8_t *puf_key)
{
    ...
    uint8_t puf_inter_response_key[PUF_RESPONSE_SIZE];
    size_t digest_len = 32;

    /* Reconstruct intrinsic key from keyCode1 generated by PUF_SetIntrinsicKey() */
    status = PUF_GetKey(PUF, key_code, PUF_KEY_CODE_SIZE, puf_inter_response_key, PUF_KEY_SIZE);
    if (status != kStatus_Success)
    {
    ...
    TEE_DigestUpdate(digest, puf_inter_response_key, PUF_RESPONSE_SIZE);
    ret = TEE_DigestDoFinal(digest, NULL, 0, puf_key, &digest_len);
    if (ret != 0) {
        TEE_FreeDigestOperation(digest);
        secure_memzero(puf_inter_response_key, 32);
        return TEE_ERROR_GENERIC;
    }
    secure_memzero(puf_inter_response_key, 32);
    return TEE_SUCCESS;
