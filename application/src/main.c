...
static int cmd_ta_commit(const struct shell *shell, size_t argc, char **argv)
{
    ...
    int res = call_puf_ta_get_commitment(tee_dev, session_id, (uint8_t *)shm_ptr, COM_x, COM_y);
    if (res) {
...
static int cmd_ta_zk(const struct shell *shell, size_t argc, char **argv)
{
    ...
    int res = call_puf_ta_get_zk_proofs(tee_dev, session_id, (uint8_t *)shm_ptr, P_x, P_y, v, w);
    if (res) {
